  ...
  pirates: [
    ...
    [
      ...
      generateDescription(
        'Флот T3-T5',
        'https://forded.github.io/ageofwater-map.github.io/icons/pirate.png',
        `Пиратский флот состояший из ${SHIPS.T5.REPEJ.name}, ${SHIPS.T4.ZABIJAKA.name}, ${SHIPS.T3.CLEVER.name}, ${SHIPS.T2.MARIANA.name} и ${SHIPS.T3.BISTRIY.name}.`,
        [
          RESPURCES.SCRAP,
          RESPURCES.FIBER,
          RESPURCES.WOOD,
...
function generateDescription(title, image = null, description = null, resources = [], boardings = [], nuances = null, actions = [], ships = []) {
  ...
  if (ships && ships.length) {
    ...
    for (const ship of ships) {
      context += `<div class="board-ship-block"><b>${ship.name}</b><img class="popup-resource-image" src="${ship.img}" width="300" height="340" /></div>`;
    }
    context += `</div>`;
  }
  return context;
